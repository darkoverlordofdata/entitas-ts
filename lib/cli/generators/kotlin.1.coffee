  ...
  run: (flags...) ->
    ...
    for Name, properties of config.components
      ...
      if properties is false 
        sc.push "var active:Boolean = false"
      else
        for p in properties
          name = p.split(':')[0]
          type = getType(p.split(':')[1])
          value = getDefault(p.split(':')[1])
          sc.push "var #{name}:#{type} = #{value}"
          
      s0.push "data class #{Name}Component(#{sc.join(', ')}) : IComponent "
    ...
    for Name, properties of config.components
      ...
      switch properties
        when false
          ...
          s2.push "val Matcher.static.#{Name}:IMatcher get() = Matcher.static.allOf(Component.#{Name}.ordinal) "
