...
utils.HashMap=HashMap})(utils=entitas.utils||(entitas.utils={}))})(entitas||(entitas={}));var entitas;
...
"reusable",{get:function(){return this._pool.reusableEntitiesCount},enumerable:true,configurable:true});return PoolObserver}();browser.PoolObserver=PoolObserver})(browser=entitas.browser||(entitas.browser={}))})(entitas||(entitas={}));var entitas;
...
undefined;this._singleEntityCache=undefined;entity.addRef()}};Group.prototype.addEntity=function(entity,index,component){if(!(entity.id in this._entities)){this._entities[entity.id]=entity;this._entitiesCache=undefined;this._singleEntityCache=undefined;entity.addRef();var onEntityAdded=this.onEntityAdded;if(onEntityAdded.active)onEntityAdded.dispatch(this,entity,index,component)}};Group.prototype.removeEntitySilently=function(entity){if(entity.id in this._entities){delete this._entities[entity.id];
this._entitiesCache=undefined;this._singleEntityCache=undefined;entity.release()}};Group.prototype.removeEntity=function(entity,index,component){if(entity.id in this._entities){delete this._entities[entity.id];this._entitiesCache=undefined;this._singleEntityCache=undefined;var onEntityRemoved=this.onEntityRemoved;if(onEntityRemoved.active)onEntityRemoved.dispatch(this,entity,index,component);entity.release()}};Group.prototype.containsEntity=function(entity){return entity.id in this._entities};Group.prototype.getEntities=
function(){if(this._entitiesCache===undefined){var entities=this._entities;var keys=Object.keys(entities);var length=keys.length;var entitiesCache=this._entitiesCache=new Array(length);for(var i=0;i<length;i++)entitiesCache[i]=entities[keys[i]]}return this._entitiesCache};Group.prototype.getSingleEntity=function(){if(this._singleEntityCache===undefined){var enumerator=Object.keys(this._entities);var c=enumerator.length;if(c===1)this._singleEntityCache=this._entities[enumerator[0]];else if(c===0)return undefined;
else throw new SingleEntityException(this._matcher);}return this._singleEntityCache};Group.prototype.toString=function(){if(this._toStringCache===undefined)this._toStringCache="Group("+this._matcher+")";return this._toStringCache};return Group}();entitas.Group=Group})(entitas||(entitas={}));var entitas;
...
new Bag;this._retainedEntities={};this._totalComponents=0;this._creationIndex=0;this.updateGroupsComponentAddedOrRemoved=function(entity,index,component){var groups=_this._groupsForIndex[index];if(groups!==undefined)for(var i=0,groupsCount=groups.size();i<groupsCount;i++)groups[i].handleEntity(entity,index,component)};this.updateGroupsComponentReplaced=function(entity,index,previousComponent,newComponent){var groups=_this._groupsForIndex[index];if(groups!==undefined)for(var i=0,groupsCount=groups.size();i<
groupsCount;i++)groups[i].updateEntity(entity,index,previousComponent,newComponent)};this.onEntityReleased=function(entity){if(entity._isEnabled)throw new EntityIsNotDestroyedException("Cannot release entity.");entity.onEntityReleased.remove(_this._cachedOnEntityReleased);delete _this._retainedEntities[entity.id];_this._reusableEntities.add(entity)};this.onGroupCreated=new Signal(this);this.onEntityCreated=new Signal(this);this.onEntityDestroyed=new Signal(this);this.onEntityWillBeDestroyed=new Signal(this);
...
this._entitiesCache=undefined;var onEntityWillBeDestroyed=this.onEntityWillBeDestroyed;if(onEntityWillBeDestroyed.active)onEntityWillBeDestroyed.dispatch(this,entity);entity.destroy();var onEntityDestroyed=this.onEntityDestroyed;if(onEntityDestroyed.active)onEntityDestroyed.dispatch(this,entity);if(entity._refCount===1){entity.onEntityReleased.remove(this._cachedOnEntityReleased);this._reusableEntities.add(entity)}else this._retainedEntities[entity.id]=entity;entity.release()};Pool.prototype.destroyAllEntities=
function(){var entities=this.getEntities();for(var i=0,entitiesLength=entities.length;i<entitiesLength;i++)this.destroyEntity(entities[i])};Pool.prototype.hasEntity=function(entity){return entity.id in this._entities};Pool.prototype.getEntities=function(){if(this._entitiesCache===undefined){var entities=this._entities;var keys=Object.keys(entities);var length=keys.length;var entitiesCache=this._entitiesCache=new Array(length);for(var i=0;i<length;i++){var k=keys[i];entitiesCache[i]=entities[k]}}return entitiesCache};
