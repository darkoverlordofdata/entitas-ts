...
return Bag}(Array);utils.Bag=Bag})(utils=entitas.utils||(entitas.utils={}))})(entitas||(entitas={}));var entitas;
(function(entitas){var utils;(function(utils){var Bag=entitas.utils.Bag;var Signal=function(){function Signal(context,alloc){if(alloc===void 0)alloc=16;this._listeners=new Bag;this._context=context;this._alloc=alloc;this.active=false}Signal.prototype.dispatch=function(){var args=[];for(var _i=0;_i<arguments.length;_i++)args[_i-0]=arguments[_i];var listeners=this._listeners;var size=listeners.size();if(size<=0)return;var context=this._context;for(var i=0;i<size;i++)listeners[i].apply(context,args)};
...
entitas.exceptions||(entitas.exceptions={}))})(entitas||(entitas={}));var entitas;
...
{}))})(entitas||(entitas={}));var entitas;(function(entitas){var exceptions;(function(exceptions){var Exception=entitas.Exception;var SingleEntityException=function(_super){__extends(SingleEntityException,_super);function SingleEntityException(matcher){_super.call(this,"Multiple entities exist matching "+matcher)}return SingleEntityException}(Exception);exceptions.SingleEntityException=SingleEntityException})(exceptions=entitas.exceptions||(entitas.exceptions={}))})(entitas||(entitas={}));var entitas;
...
var length_1=keys.length;var entitiesCache=this._entitiesCache=new Array(length_1);for(var i=0;i<length_1;i++)entitiesCache[i]=entities[keys[i]]}return this._entitiesCache};Group.prototype.getSingleEntity=function(){if(this._singleEntityCache==null){var enumerator=Object.keys(this._entities);var c=enumerator.length;if(c===1)this._singleEntityCache=this._entities[enumerator[0]];else if(c===0)return null;else throw new SingleEntityException(this._matcher);}return this._singleEntityCache};Group.prototype.toString=
...
group.onEntityAdded.add(this._addEntityCache);group.onEntityRemoved.remove(this._addEntityCache);group.onEntityRemoved.add(this._addEntityCache)}else throw"Invalid eventType ["+typeof eventType+":"+eventType+"] in GroupObserver::activate";}};GroupObserver.prototype.deactivate=function(){for(var i=0,groupsLength=this._groups.length;i<groupsLength;i++){var group=this._groups[i];group.onEntityAdded.remove(this._addEntityCache);group.onEntityRemoved.remove(this._addEntityCache);this.clearCollectedEntities()}};
...
(function(entitas){var UUID=entitas.utils.UUID;var Bag=entitas.utils.Bag;var Group=entitas.Group;var Entity=entitas.Entity;var Signal=entitas.utils.Signal;var EntityIsNotDestroyedException=entitas.exceptions.EntityIsNotDestroyedException;var PoolDoesNotContainEntityException=entitas.exceptions.PoolDoesNotContainEntityException;function as(obj,method1){return method1 in obj?obj:null}var Pool=function(){function Pool(components,totalComponents,debug,startCreationIndex){var _this=this;if(debug===void 0)debug=
false;if(startCreationIndex===void 0)startCreationIndex=0;this.onEntityCreated=null;this.onEntityWillBeDestroyed=null;this.onEntityDestroyed=null;this.onGroupCreated=null;this.name="";this._debug=false;this._entities={};this._groups={};this._groupsForIndex=null;this._reusableEntities=new Bag;this._retainedEntities={};this._componentsEnum=null;this._totalComponents=0;this._creationIndex=0;this._entitiesCache=null;this.updateGroupsComponentAddedOrRemoved=function(entity,index,component){var groups=
_this._groupsForIndex[index];if(groups!=null)for(var i=0,groupsCount=groups.size();i<groupsCount;i++)groups[i].handleEntity(entity,index,component)};this.updateGroupsComponentReplaced=function(entity,index,previousComponent,newComponent){var groups=_this._groupsForIndex[index];if(groups!=null)for(var i=0,groupsCount=groups.size();i<groupsCount;i++)groups[i].updateEntity(entity,index,previousComponent,newComponent)};this.onEntityReleased=function(entity){if(entity._isEnabled)throw new EntityIsNotDestroyedException("Cannot release entity.");
entity.onEntityReleased.remove(_this._cachedOnEntityReleased);delete _this._retainedEntities[entity.id];_this._reusableEntities.add(entity)};Pool.instance=this;this.onGroupCreated=new Signal(this);this.onEntityCreated=new Signal(this);this.onEntityDestroyed=new Signal(this);this.onEntityWillBeDestroyed=new Signal(this);this._debug=debug;this._componentsEnum=components;this._totalComponents=totalComponents;this._creationIndex=startCreationIndex;this._groupsForIndex=new Bag;this._cachedUpdateGroupsComponentAddedOrRemoved=
this.updateGroupsComponentAddedOrRemoved;this._cachedUpdateGroupsComponentReplaced=this.updateGroupsComponentReplaced;this._cachedOnEntityReleased=this.onEntityReleased;Pool.componentsEnum=components;Pool.totalComponents=totalComponents}Object.defineProperty(Pool.prototype,"totalComponents",{get:function(){return this._totalComponents},enumerable:true,configurable:true});Object.defineProperty(Pool.prototype,"count",{get:function(){return Object.keys(this._entities).length},enumerable:true,configurable:true});
Object.defineProperty(Pool.prototype,"reusableEntitiesCount",{get:function(){return this._reusableEntities.size()},enumerable:true,configurable:true});Object.defineProperty(Pool.prototype,"retainedEntitiesCount",{get:function(){return Object.keys(this._retainedEntities).length},enumerable:true,configurable:true});Pool.setPool=function(system,pool){var poolSystem=as(system,"setPool");if(poolSystem!=null)poolSystem.setPool(pool)};Pool.prototype.createEntity=function(name){var entity=this._reusableEntities.size()>
0?this._reusableEntities.removeLast():new Entity(this._componentsEnum,this._totalComponents);entity._isEnabled=true;entity.name=name;entity._creationIndex=this._creationIndex++;entity.id=UUID.randomUUID();entity.addRef();this._entities[entity.id]=entity;this._entitiesCache=null;entity.onComponentAdded.add(this._cachedUpdateGroupsComponentAddedOrRemoved);entity.onComponentRemoved.add(this._cachedUpdateGroupsComponentAddedOrRemoved);entity.onComponentReplaced.add(this._cachedUpdateGroupsComponentReplaced);
entity.onEntityReleased.add(this._cachedOnEntityReleased);var onEntityCreated=this.onEntityCreated;if(onEntityCreated.active)onEntityCreated.dispatch(this,entity);return entity};Pool.prototype.destroyEntity=function(entity){if(!(entity.id in this._entities))throw new PoolDoesNotContainEntityException(entity,"Could not destroy entity!");delete this._entities[entity.id];this._entitiesCache=null;var onEntityWillBeDestroyed=this.onEntityWillBeDestroyed;if(onEntityWillBeDestroyed.active)onEntityWillBeDestroyed.dispatch(this,
entity);entity.destroy();var onEntityDestroyed=this.onEntityDestroyed;if(onEntityDestroyed.active)onEntityDestroyed.dispatch(this,entity);if(entity._refCount===1){entity.onEntityReleased.remove(this._cachedOnEntityReleased);this._reusableEntities.add(entity)}else this._retainedEntities[entity.id]=entity;entity.release()};Pool.prototype.destroyAllEntities=function(){var entities=this.getEntities();for(var i=0,entitiesLength=entities.length;i<entitiesLength;i++)this.destroyEntity(entities[i])};Pool.prototype.hasEntity=
function(entity){return entity.id in this._entities};Pool.prototype.getEntities=function(matcher){if(matcher)return this.getGroup(matcher).getEntities();else{if(this._entitiesCache==null){var entities=this._entities;var keys=Object.keys(entities);var length_2=keys.length;var entitiesCache=this._entitiesCache=new Array(length_2);for(var i=0;i<length_2;i++)entitiesCache[i]=entities[keys[i]]}return this._entitiesCache}};Pool.prototype.createSystem=function(system){if("function"===typeof system){var Klass=
system;system=new Klass}Pool.setPool(system,this);var reactiveSystem=as(system,"trigger");if(reactiveSystem!=null)return new entitas.ReactiveSystem(this,reactiveSystem);var multiReactiveSystem=as(system,"triggers");if(multiReactiveSystem!=null)return new entitas.ReactiveSystem(this,multiReactiveSystem);return system};Pool.prototype.getGroup=function(matcher){var group;if(matcher.id in this._groups)group=this._groups[matcher.id];else{group=new Group(matcher);var entities=this.getEntities();for(var i=
0,entitiesLength=entities.length;i<entitiesLength;i++)group.handleEntitySilently(entities[i]);this._groups[matcher.id]=group;for(var i=0,indicesLength=matcher.indices.length;i<indicesLength;i++){var index=matcher.indices[i];if(this._groupsForIndex[index]==null)this._groupsForIndex[index]=new Bag;this._groupsForIndex[index].add(group)}var onGroupCreated=this.onGroupCreated;if(onGroupCreated.active)onGroupCreated.dispatch(this,group)}return group};Pool.componentsEnum=null;Pool.totalComponents=0;Pool.instance=
null;return Pool}();entitas.Pool=Pool})(entitas||(entitas={}));var entitas;
...
proxy.name).listen()});pool.onEntityDestroyed.add(function(pool,entity){var controller=VisualDebugging._controllers[entity.id];delete VisualDebugging._controllers[entity.id];VisualDebugging._entities.remove(controller)});var superInitialize_1=Systems.prototype.initialize;Systems.prototype.initialize=function(){superInitialize_1.call(this);var sys=new SystemObserver(this);VisualDebugging._systems.add(sys,"initialize").listen();VisualDebugging._systems.add(sys,"execute").listen()};function get_Systems(){return"Systems "+
" ("+this._initializeSystems.length+" init, "+this._executeSystems.length+" exe "}Object.defineProperty(Systems.prototype,"name",{get:function(){return"Systems"}});Object.defineProperty(Systems.prototype,"Systems",{get:get_Systems})}};return VisualDebugging}();viewer.VisualDebugging=VisualDebugging})(viewer=entitas.viewer||(entitas.viewer={}))})(entitas||(entitas={}));
