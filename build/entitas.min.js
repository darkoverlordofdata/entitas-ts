...
(function(entitas){var EntityIsNotEnabledException=entitas.EntityIsNotEnabledException;var EntityIsAlreadyReleasedException=entitas.EntityIsAlreadyReleasedException;var EntityAlreadyHasComponentException=entitas.EntityAlreadyHasComponentException;var EntityDoesNotHaveComponentException=entitas.EntityDoesNotHaveComponentException;var Entity=function(){function Entity(componentsEnum,totalComponents){if(totalComponents===void 0)totalComponents=16;this._creationIndex=0;this._isEnabled=true;this._refCount=
0;this._componentsEnum=componentsEnum;if(Entity.instanceIndex===0)Entity.dim(totalComponents,100);this._pool=entitas.Pool.instance;this.instanceIndex=Entity.instanceIndex++;this._components=Entity.alloc[this.instanceIndex]}Object.defineProperty(Entity.prototype,"creationIndex",{get:function(){return this._creationIndex},enumerable:true,configurable:true});Entity.dim=function(count,size){if(!Entity.first)return;Entity.first=false;Entity.alloc=new Array(size);for(var e=0;e<size;e++){Entity.alloc[e]=
new Array(count);for(var k=0;k<count;k++)Entity.alloc[e][k]=null}};Entity.prototype.addComponent=function(index,component){if(!this._isEnabled)throw new EntityIsNotEnabledException("Cannot add component!");if(this.hasComponent(index)){var errorMsg="Cannot add component at index "+index+" to "+this;throw new EntityAlreadyHasComponentException(errorMsg,index);}this._components[index]=component;this._componentsCache=null;this._componentIndicesCache=null;this._toStringCache=null;this._pool.updateGroupsComponentAddedOrRemoved(this,
...
index,previousComponent,newComponent)};this.onEntityReleased=function(entity){if(entity._isEnabled)throw new EntityIsNotDestroyedException("Cannot release entity.");delete _this._retainedEntities[entity.id];_this._reusableEntities.add(entity)};Pool.instance=this;this.onGroupCreated=new Signal(this);this.onEntityCreated=new Signal(this);this.onEntityDestroyed=new Signal(this);this.onEntityWillBeDestroyed=new Signal(this);this._componentsEnum=components;this._totalComponents=totalComponents;this._creationIndex=
startCreationIndex;this._groupsForIndex=new Bag;this._cachedUpdateGroupsComponentAddedOrRemoved=this.updateGroupsComponentAddedOrRemoved;this._cachedUpdateGroupsComponentReplaced=this.updateGroupsComponentReplaced;this._cachedOnEntityReleased=this.onEntityReleased;Pool.componentsEnum=components;Pool.totalComponents=totalComponents}Object.defineProperty(Pool.prototype,"totalComponents",{get:function(){return this._totalComponents},enumerable:true,configurable:true});Object.defineProperty(Pool.prototype,
"count",{get:function(){return Object.keys(this._entities).length},enumerable:true,configurable:true});Object.defineProperty(Pool.prototype,"reusableEntitiesCount",{get:function(){return this._reusableEntities.size()},enumerable:true,configurable:true});Object.defineProperty(Pool.prototype,"retainedEntitiesCount",{get:function(){return Object.keys(this._retainedEntities).length},enumerable:true,configurable:true});Pool.groupDesc=function(group){var s=group.toString();for(var c=Pool.totalComponents;c>
-1;c--)s=s.replace(""+c,Pool.componentsEnum[c]);return s};Pool.prototype.createEntity=function(name){var entity=this._reusableEntities.size()>0?this._reusableEntities.removeLast():new Entity(this._componentsEnum,this._totalComponents);entity._isEnabled=true;entity.name=name;entity._creationIndex=this._creationIndex++;entity.id=UUID.randomUUID();entity.addRef();this._entities[entity.id]=entity;this._entitiesCache=null;var onEntityCreated=this.onEntityCreated;if(onEntityCreated.active)onEntityCreated.dispatch(this,
...
if(entity._refCount===1)this._reusableEntities.add(entity);else this._retainedEntities[entity.id]=entity;entity.release()};Pool.prototype.destroyAllEntities=function(){var entities=this.getEntities();for(var i=0,entitiesLength=entities.length;i<entitiesLength;i++)this.destroyEntity(entities[i])};Pool.prototype.hasEntity=function(entity){return entity.id in this._entities};Pool.prototype.getEntities=function(){if(this._entitiesCache==null){var entities=this._entities;var keys=Object.keys(entities);
var length=keys.length;var entitiesCache=this._entitiesCache=new Array(length);for(var i=0;i<length;i++){var k=keys[i];entitiesCache[i]=entities[k]}}return entitiesCache};Pool.prototype.getGroup=function(matcher){var group;if(matcher.id in this._groups)group=this._groups[matcher.id];else{group=new Group(matcher);var entities=this.getEntities();for(var i=0,entitiesLength=entities.length;i<entitiesLength;i++)group.handleEntitySilently(entities[i]);this._groups[matcher.id]=group;for(var i=0,indicesLength=
matcher.indices.length;i<indicesLength;i++){var index=matcher.indices[i];if(this._groupsForIndex[index]==null)this._groupsForIndex[index]=new Bag;this._groupsForIndex[index].add(group)}var onGroupCreated=this.onGroupCreated;if(onGroupCreated.active)onGroupCreated.dispatch(this,group)}return group};Pool.totalComponents=0;return Pool}();entitas.Pool=Pool})(entitas||(entitas={}));var entitas;
