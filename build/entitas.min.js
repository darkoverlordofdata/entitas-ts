...
(function(entitas){var Bag=function(_super){__extends(Bag,_super);function Bag(capacity){if(capacity===void 0)capacity=64;_super.call(this,capacity);this.size_=0}Bag.prototype.removeAt=function(index){var e=this[index];this[index]=this[--this.size_];this[this.size_]=null;return e};Bag.prototype.remove=function(e){var i;var e2;var size=this.size_;for(i=0;i<size;i++){e2=this[i];if(e==e2){this[i]=this[--this.size_];this[this.size_]=null;return true}}return false};Bag.prototype.removeLast=function(){if(this.size_>
0){var e=this[--this.size_];this[this.size_]=null;return e}return null};Bag.prototype.contains=function(e){var i;var size;for(i=0,size=this.size_;size>i;i++)if(e===this[i])return true;return false};Bag.prototype.removeAll=function(bag){var modified=false;var i;var j;var l;var e1;var e2;for(i=0,l=bag.size();i<l;i++){e1=bag.get(i);for(j=0;j<this.size_;j++){e2=this[j];if(e1===e2){this.removeAt(j);j--;modified=true;break}}}return modified};Bag.prototype.get=function(index){if(index>=this.length)throw new Error("ArrayIndexOutOfBoundsException");
return this[index]};Bag.prototype.safeGet=function(index){if(index>=this.length)this.grow(index*7/4+1);return this[index]};Bag.prototype.size=function(){return this.size_};Bag.prototype.getCapacity=function(){return this.length};Bag.prototype.isIndexWithinBounds=function(index){return index<this.getCapacity()};Bag.prototype.isEmpty=function(){return this.size_==0};Bag.prototype.add=function(e){if(this.size_===this.length)this.grow();this[this.size_++]=e};Bag.prototype.set=function(index,e){if(index>=
this.length)this.grow(index*2);this.size_=index+1;this[index]=e};Bag.prototype.grow=function(newCapacity){if(newCapacity===void 0)newCapacity=~~(this.length*3/2)+1;this.length=~~newCapacity};Bag.prototype.ensureCapacity=function(index){if(index>=this.length)this.grow(index*2)};Bag.prototype.clear=function(){var i;var size;for(i=0,size=this.size_;i<size;i++)this[i]=null;this.size_=0};Bag.prototype.addAll=function(items){var i;for(i=0;items.size()>i;i++)this.add(items.get(i))};return Bag}(Array);entitas.Bag=
Bag})(entitas||(entitas={}));var entitas;
(function(entitas){var Exception=function(){function Exception(message){this.message=message}Exception.prototype.toString=function(){return this.message};return Exception}();entitas.Exception=Exception;var EntityAlreadyHasComponentException=function(_super){__extends(EntityAlreadyHasComponentException,_super);function EntityAlreadyHasComponentException(message,index){_super.call(this,message+"\nEntity already has a component at index "+index)}return EntityAlreadyHasComponentException}(Exception);entitas.EntityAlreadyHasComponentException=
EntityAlreadyHasComponentException;var EntityDoesNotHaveComponentException=function(_super){__extends(EntityDoesNotHaveComponentException,_super);function EntityDoesNotHaveComponentException(message,index){_super.call(this,message+"\nEntity does not have a component at index "+index)}return EntityDoesNotHaveComponentException}(Exception);entitas.EntityDoesNotHaveComponentException=EntityDoesNotHaveComponentException;var EntityIsNotEnabledException=function(_super){__extends(EntityIsNotEnabledException,
...
(entitas={}));var entitas;
(function(entitas){var Bag=entitas.Bag;var Signal=function(){function Signal(context,alloc){if(alloc===void 0)alloc=16;this._listeners=new Bag;this._context=context;this._alloc=alloc;this._size=0}Signal.prototype.dispatch=function(){var args=[];for(var _i=0;_i<arguments.length;_i++)args[_i-0]=arguments[_i];var listeners=this._listeners;var size=listeners.size();var context=this._context;for(var i=0;i<size;i++)listeners[i].apply(context,args)};Signal.prototype.add=function(listener){this._listeners.add(listener)};Signal.prototype.remove=
function(listener){this._listeners.remove(listener)};Signal.prototype.clear=function(){this._listeners.clear()};return Signal}();entitas.Signal=Signal})(entitas||(entitas={}));var entitas;
...
this._entitiesCache=undefined;this._singleEntityCache=undefined;this.onEntityRemoved.dispatch(this,entity,index,component);entity.release()}};Group.prototype.containsEntity=function(entity){return entity.creationIndex in this._entities};Group.prototype.getEntities=function(){if(this._entitiesCache===undefined){var entities=this._entities;var keys=Object.keys(entities);var length=keys.length;var entitiesCache=this._entitiesCache=new Array(length);for(var i=0;i<length;i++)entitiesCache[i]=entities[keys[i]]}return this._entitiesCache};
Group.prototype.getSingleEntity=function(){if(this._singleEntityCache===undefined){var enumerator=Object.keys(this._entities);var c=enumerator.length;if(c===1)this._singleEntityCache=this._entities[enumerator[0]];else if(c===0)return undefined;else throw new SingleEntityException(this._matcher);}return this._singleEntityCache};Group.prototype.toString=function(){if(this._toStringCache===undefined)this._toStringCache="Group("+this._matcher+")";return this._toStringCache};return Group}();entitas.Group=
Group})(entitas||(entitas={}));var entitas;
...
undefined;this.onEntityWillBeDestroyed.dispatch(this,entity);entity.destroy();this.onEntityDestroyed.dispatch(this,entity);if(entity._refCount===1){entity.onEntityReleased.remove(this._cachedOnEntityReleased);this._reusableEntities.add(entity)}else this._retainedEntities[entity.creationIndex]=entity;entity.release()};Pool.prototype.destroyAllEntities=function(){var entities=this.getEntities();for(var i=0,entitiesLength=entities.length;i<entitiesLength;i++)this.destroyEntity(entities[i])};Pool.prototype.hasEntity=
function(entity){return entity.creationIndex in this._entities};Pool.prototype.getEntities=function(){if(this._entitiesCache===undefined){var entities=this._entities;var keys=Object.keys(entities);var length=keys.length;var entitiesCache=this._entitiesCache=new Array(length);for(var i=0;i<length;i++){var k=keys[i];entitiesCache[i]=entities[k]}}return entitiesCache};Pool.prototype.getGroup=function(matcher){var group;if(matcher.id in this._groups)group=this._groups[matcher.id];else{group=new Group(matcher);
var entities=this.getEntities();for(var i=0,entitiesLength=entities.length;i<entitiesLength;i++)group.handleEntitySilently(entities[i]);this._groups[matcher.id]=group;for(var i=0,indicesLength=matcher.indices.length;i<indicesLength;i++){var index=matcher.indices[i];if(this._groupsForIndex[index]===undefined)this._groupsForIndex[index]=new Bag;this._groupsForIndex[index].add(group)}this.onGroupCreated.dispatch(this,group)}return group};Pool.totalComponents=0;return Pool}();entitas.Pool=Pool})(entitas||
(entitas={}));var entitas;
