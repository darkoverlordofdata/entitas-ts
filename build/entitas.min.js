...
function(entity){return entity.id in this._entities};Pool.prototype.getEntities=function(){if(this._entitiesCache==null){var entities=this._entities;var keys=Object.keys(entities);var length=keys.length;var entitiesCache=this._entitiesCache=new Array(length);for(var i=0;i<length;i++){var k=keys[i];entitiesCache[i]=entities[k]}}return entitiesCache};Pool.prototype.getGroup=function(matcher){var group;if(matcher.id in this._groups)group=this._groups[matcher.id];else{group=new Group(matcher);var entities=
this.getEntities();for(var i=0,entitiesLength=entities.length;i<entitiesLength;i++)group.handleEntitySilently(entities[i]);this._groups[matcher.id]=group;for(var i=0,indicesLength=matcher.indices.length;i<indicesLength;i++){var index=matcher.indices[i];if(this._groupsForIndex[index]==null)this._groupsForIndex[index]=new Bag;this._groupsForIndex[index].add(group)}var onGroupCreated=this.onGroupCreated;if(onGroupCreated.active)onGroupCreated.dispatch(this,group)}return group};Pool.totalComponents=0;
return Pool}();entitas.Pool=Pool})(entitas||(entitas={}));var entitas;
